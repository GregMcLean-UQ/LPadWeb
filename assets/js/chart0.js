...
function drawMetChart() {
	...
	$.getJSON('jsonp.php?table=0&chartType=' + chartType + '&callback=?', function(data) {
		...
		var metData = new Array();
			...
			var seriesNames = new Array("Humidity","Temperature","Radiation");
			var ax = new Array(1,1,2);
			for(var i=0;i<3;i++){
				seriesOptions.push({name : seriesNames[i], data : metData[i]});
            }
